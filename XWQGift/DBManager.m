...
- (void)insertModel:(id)model recordType:(NSString *)type {
    GiftDataModel *appModel = (GiftDataModel *)model;
    
    if ([self isExistAppForAppId:appModel.idt recordType:type]) {
        NSLog(@"this app has  recorded");
        return;
    }
    NSString *sql = @"insert into appInfo(appName,appId,currentPrice,lastPrice,recordType) values (?,?,?,?,?)";
    BOOL isSuccess = [_database executeUpdate:sql,appModel.idt,appModel.name,appModel.photo,appModel.desc,type];
    if (!isSuccess) {
...
- (void)deleteModelForAppId:(NSString *)appId recordType:(NSString *)type {
    NSString *sql = @"delete from appInfo where appName = ? and recordType = ?";
...
- (NSArray *)readModelsWithRecordType:(NSString *)type{
    ...
    while ([rs next]) {
        //把查询之后结果 放在model
        GiftDataModel *appModel = [[GiftDataModel alloc] init];
        appModel.idt = [rs stringForColumn:@"appName"];
        appModel.name = [rs stringForColumn:@"appId"];
        appModel.photo = [rs stringForColumn:@"currentPrice"];
        appModel.desc = [rs stringForColumn:@"lastPrice"];
...
- (BOOL)isExistAppForAppId:(NSString *)appId recordType:(NSString *)type {
    NSString *sql = @"select * from appInfo where appName = ? and recordType = ?";
